...
class SelfSubjectReviewTest {
  ...
  void deserializationAndSerializationShouldWorkAsExpected() throws IOException {
    // Given
    String originalJson = new Scanner(getClass().getResourceAsStream("/valid-selfsubjectreview-v1.json"))
        .useDelimiter("\\A")
        .next();
    ...
    assertThat(selfSubjectReview)
        ...
        .hasFieldOrPropertyWithValue("status.userInfo.username", "usernameValue")
        .hasFieldOrPropertyWithValue("status.userInfo.uid", "uidValue")
        .hasFieldOrPropertyWithValue("status.userInfo.groups", Arrays.asList("groupsValue"));
  }
