...
class WasmPluginTest {
  ...
  void deserializationAndSerializationShouldWorkAsExpected() throws JacksonException {
    ...
    assertThat(wasmPlugin)
        ...
        .hasFieldOrPropertyWithValue("spec.pluginConfig.value.openid_server", "authn")
        .hasFieldOrPropertyWithValue("spec.pluginConfig.value.openid_realm", "ingress");
  }
  ...
  void builderShouldCreateObjects() {
    ...
    assertThat(wasmPlugin)
        ...
        .hasFieldOrPropertyWithValue("spec.url", "file:///opt/filters/openid.wasm")
        .hasFieldOrPropertyWithValue("spec.sha256", "1ef0c9a92b0420cf25f7fe5d481b231464bc88f486ca3b9c83ed5cc21df6210")
        .extracting("spec.pluginConfig").asInstanceOf(InstanceOfAssertFactories.map(String.class, String.class))
        .containsOnly(
            entry("openid_server", "authn"),
            entry("openid_realm", "ingress"));
  }
}
