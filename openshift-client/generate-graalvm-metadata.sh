...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  ...
  # Copy generated metadata (except native-image.properties which is manually configured)
  ...
  for file in reflect-config.json jni-config.json resource-config.json serialization-config.json; do
    if [ -f "$AGENT_OUTPUT_DIR/$file" ]; then
      cp "$AGENT_OUTPUT_DIR/$file" "$METADATA_DIR/$file"
      echo "  ✓ $file"
    fi
  done

  echo ""
  echo "⚠️  NOTE: native-image.properties NOT copied (manually configured)"
  echo ""
